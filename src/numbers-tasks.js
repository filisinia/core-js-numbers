...
function getAverage(value1, value2) {
  ...
  return value1.toExponential() / 2 + value2.toExponential() / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthOfA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthOfB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAB = scalarValue / (lengthOfA * lengthOfB);
